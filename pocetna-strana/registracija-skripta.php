<?php
    ...
    $upit1 = "INSERT INTO korisnici(ime, prezime, korisnicko_ime, lozinka, telefon, email, tip_korisnika, status) VALUES('$ime','$prezime','$kor_ime','$lozinka','$telefon','$email',$tip, 0)";
    ...
    echo '<a href="./pocetna.php">Vratite se na početnu stranu i sačekajte da Vam administrator prihvati zahtev za registraciju.</a> U međuvremenu možete da koristite sajt kao neregistrovani korisnik.';
    
    if (!empty($_POST['profilna_slika'])) {
        ...
        if(!in_array($ekstenzija, $tipovi)) {
            ...
            echo "<a href='./registracija.php'>Vrati se na prethodnu stranicu</a><br />";
            $fleg = false;
        }
        ...
        if($flag) {
            if(move_uploaded_file($temp, $putanja)) {
                $upit_slika = "UPDATE korisnici SET slika='$ime_slike' WHERE korisnicko_ime'".$ime."'";
                $status_slika = mysqli_query($konekcija, $upit_slika);
            } else {
